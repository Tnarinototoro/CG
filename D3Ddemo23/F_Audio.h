...
//Play wav functions definitions
class F_Audio
{
public:
	F_Audio();
	...
	void StopAudioNow()
	{
		this->pSourceVoice->Discontinuity();
	}
	void PauseAudio()
	{
	...
	HRESULT InitSound(IXAudio2* pXaudio2, LPWSTR szFilename);
	//This function will firstly submit the buffers then play the sound
	void PlaysoundTotheend()
	{
	...
	IXAudio2 * Getm_Xaudio()
	{
		return this->m_Xaudio;
	}
...
F_Audio::F_Audio()
{
	...
	hr = S_OK;
	HRESULT hrError;
	if (FAILED(hrError = XAudio2Create(&m_Xaudio, XaudioFlags)))
	{
		wprintf(L"Failed to init XAudio2 engine: %#X\n", hrError);
		CoUninitialize();
	}
	//
	// init a mastering voice
	//
	IXAudio2MasteringVoice* pMasteringVoice = NULL;

	if (FAILED(hrError = m_Xaudio->CreateMasteringVoice(&m_MasterVoice)))
	{
...
HRESULT F_Audio::InitSound(IXAudio2* pXaudio2, LPWSTR FilePath)
{
	//
	// Read in the wave file
	//
	
	if (FAILED(hr = wav.Open(FilePath, NULL, WAVEFILE_READ)))
	{
